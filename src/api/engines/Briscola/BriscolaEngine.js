...
class BriscolaEngine {
    ...
    endRound() {
        ...
        this.roundCards.forEach(cardPlayed => {
            ...
            if (!highestCard || Values[cardPlayed.card.value] > Values[highestCard.card.value]) {
                highestCard = cardPlayed;
            }
        });
        const winner = this.players.find(player => player.id === highestCard.player.id);
        winner.cardsWon.push(...this.roundCards.map(cardPlayed => new Card(cardPlayed.card.suit, cardPlayed.card.value, cardPlayed.card.points)));
        this.nextRound();
    }
